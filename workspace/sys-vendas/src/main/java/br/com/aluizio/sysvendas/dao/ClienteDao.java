...
public class ClienteDao {
	...
	public void adicionaAltera(Cliente cliente) {
		String sql;
		if(cliente.getId()==null) {
			sql = "insert into Clientes (pessoa, situacao,"
				+ " sexo, nome, sobreNome, nascimento, "
				+ " email, celular, endereco, bairro, cidade, "
				+ " complemento, numero, uf, cep, fone, observacao) "
				+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		}else {
			sql = "update Clientes set pessoa=?, situacao=?,"
				+ " sexo=?, nome=?, sobreNome=?, nascimento=?, "
				+ " email=?, celular=?, endereco=?, bairro=?, cidade=?, "
				+ " complemento=?, numero=?, uf=?, cep=?, fone=?, observacao=? "
